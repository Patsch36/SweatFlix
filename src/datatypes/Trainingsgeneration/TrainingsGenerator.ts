...
export class TrainingsGenerator {
  ...
  private async saveToDatabase(
    ...
    for (let workout in workoutTemplate) {
      ...
      const countQuery = `SELECT COUNT(Name) as count FROM WorkoutTemplate WHERE Name = '${workoutTemplate[workout].workoutName}-${datestring}'`;
      const countResult = await this.db.query(countQuery);
      const count = countResult ? countResult.values[0].count : 0;
      console.log(
        `Number of workouttemplates with name ${workoutTemplate[workout].workoutName}-${datestring}: ${count}`
      );

      const workouttemplatename = `${
        workoutTemplate[workout].workoutName + "-" + datestring
      }${count > 0 ? "- Part " + (parseInt(count) + 1).toString() : ""}`;

      const workouttemplateqery = `INSERT INTO WorkoutTemplate (Name, Split, Description, Color, active) VALUES ('${workouttemplatename}', '${workoutTemplate[workout].workoutName}', 'This Workout Template is autogenerated from ${datestring}.', '${randomColor}', 1)`;
      console.log(workouttemplateqery);
      await this.db.run(workouttemplateqery);

      // ===== WORKOUT TEMPLATE PLAN ===============================================

      const insertWorkoutTemplatePlanQuery = `INSERT INTO WorkoutTemplatePlan (WorkoutTemplateName, PlanID, OrderIndex) VALUES ('${workouttemplatename}', (SELECT ID FROM Plan WHERE name = 'Autogenerated Plan from ${datestring}'), ${workout})`;
      ...
      for (let exercise in workoutTemplate[workout].exercises) {
        const insertExerciseQuery = `INSERT INTO WorkoutList (OrderIndex, workoutPlan, exerciseName, sets, reps) VALUES (${exercise}, '${workouttemplatename}', '${workoutTemplate[workout].exercises[exercise].exercisename}', ${workoutTemplate[workout].exercises[exercise].sets}, '${workoutTemplate[workout].exercises[exercise].reps}')`;
